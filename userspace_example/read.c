...
#define DEVICE_FILE "/dev/rfid-reader"
...
int issue_ioctl_command(int device_fd, enum ioctl_cmd cmd, int *arg)
{
...
void device_read(int device_fd)
{
	...
	printf("[rfid reader] bytes read: %d\n", bytes_read);
